...
interface IUserIdentityRegistry {
    function getUserProfile(address _user) external view returns (
        bytes32, bytes32, bytes32, uint8, uint8, uint8, uint256, string memory, 
        uint256, bool, bytes32, bool
    );
}
...
contract GlobalCredentialAnchor is AccessControl, ReentrancyGuard {
    ...
    enum RecognitionLevel { LOCAL, REGIONAL, GLOBAL }
    ...
    event CredentialAnchored(uint256 indexed credentialId, bytes32 indexed credentialHash, address indexed holder, uint256 timestamp);
    ...
    function revokeCredential(uint256 _credentialId) external nonReentrant {
        ...
        require(
            globalCredentials[_credentialId].holder == msg.sender ||
            hasRole(ANCHOR_ADMIN_ROLE, msg.sender),
            "Not authorized"
        );
        
        globalCredentials[_credentialId].status = CredentialStatus.REVOKED;
        
        verificationLogger.logVerification(globalCredentials[_credentialId].holder, "CREDENTIAL_REVOKED", true, "Global credential revoked");
