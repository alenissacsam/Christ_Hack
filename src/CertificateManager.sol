...
contract CertificateManager is AccessControl, ReentrancyGuard {
    ...
    event CertificateIssued(uint256 indexed certId, address indexed issuer, address indexed recipient, string certType, uint256 timestamp);
    ...
    function revokeCertificate(uint256 _certId) external nonReentrant {
        ...
        require(
            certificates[_certId].issuer == msg.sender || hasRole(ADMIN_ROLE, msg.sender),
            "Not authorized to revoke"
        );
        
        certificates[_certId].isActive = false;
        
        verificationLogger.logVerification(certificates[_certId].recipient, "CERTIFICATE_REVOKED", true, "Certificate revoked");
    ...
    function getActiveCertificates(address _user) external view returns (Certificate[] memory) {
        ...
        for (uint i = 0; i < certIds.length; i++) {
            if (certificates[certIds[i]].isActive && 
    ...
    function verifyCertificate(uint256 _certId) external view returns (bool isValid, Certificate memory cert) {
        ...
        bool valid = certificate.isActive && 
    ...
    function _checkRecipientEligibility(address _recipient, string memory _certificateType) internal view returns (bool) {
        ...
        if (keccak256(bytes(_certificateType)) == keccak256(bytes("EMPLOYMENT")) ||
