...
contract RecognitionManager is AccessControl {
    ...
    event CountryRecognition(
        uint256 indexed credentialId, string country, address indexed recognizer, uint256 timestamp
    );
    ...
    function addCountryRecognition(uint256 _credentialId, string memory _country, string memory _recognitionDocument)
        external
        onlyRole(RECOGNITION_ADMIN_ROLE)
    {
    ...
    function revokeCountryRecognition(uint256 _credentialId, string memory _country)
        external
        onlyRole(RECOGNITION_ADMIN_ROLE)
    {
        RecognitionRecord[] storage recognitions = credentialRecognitions[_credentialId];

        for (uint256 i = 0; i < recognitions.length; i++) {
