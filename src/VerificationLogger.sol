...
contract VerificationLogger is AccessControl {
    ...
    function logVerification(address _user, string memory _verificationType, bool _success, string memory _details)
        external
    ...
    function getLogsByTypeAndDateRange(string memory _verificationType, uint256 _startTime, uint256 _endTime)
        external
        view
        returns (VerificationLog[] memory)
    {
    ...
    function getDailyStats(uint256 _dayTimestamp)
        external
        view
        returns (uint256 totalLogs, uint256 successCount, uint256 failureCount)
    {
