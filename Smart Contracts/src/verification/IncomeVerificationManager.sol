    ...
    UUPSUpgradeable
{
    ...
    function requestIncomeVerification(
        ...
        require(bytes(verificationSource).length > 0, "Empty verification source");
        require(uint256(incomeRange) <= 4, "Invalid income range");

        bytes32 sourceHash = keccak256(bytes(verificationSource));

        require(
            sourceHash == keccak256("ITR") || sourceHash == keccak256("BankStatement")
                || sourceHash == keccak256("SalarySlip") || sourceHash == keccak256("GST"),
        ...
        verificationLogger.logEvent("INCOME_VERIFICATION_REQUESTED", msg.sender, incomeProofHash);

        emit IncomeVerificationRequested(msg.sender, incomeRange);
    }

    function completeIncomeVerification(address user, bool success, bytes memory oracleSignature)
        public
        onlyRole(INCOME_ORACLE_ROLE)
    {
        ...
        if (success) {
            ...
            trustScore.updateScore(user, int256(INCOME_VERIFICATION_SCORE + bonus), "Income verification completed");
    ...
    function renewIncomeVerification(
        ...
        verificationLogger.logEvent("INCOME_VERIFICATION_RENEWAL_REQUESTED", msg.sender, newIncomeProofHash);

        emit IncomeVerificationRenewed(msg.sender, newIncomeRange);
    }

    function revokeIncomeVerification(address user, string memory reason) external onlyRole(VERIFIER_ROLE) {
        require(incomeVerifications[user].isVerified, "Income not verified");

        uint256 deductionAmount =
            INCOME_VERIFICATION_SCORE + _calculateIncomeBonus(incomeVerifications[user].incomeRange);
        ...
        verificationLogger.logEvent("INCOME_VERIFICATION_REVOKED", user, keccak256(bytes(reason)));
    ...
    function bulkCompleteIncomeVerification(address[] memory users, bool[] memory successes, bytes[] memory signatures)
        external
        onlyRole(INCOME_ORACLE_ROLE)
    {
    ...
    function isIncomeVerified(address user) external view returns (bool) {
        return incomeVerifications[user].isVerified && incomeVerifications[user].isActive
            && !_isIncomeVerificationExpired(user);
    }

    function getIncomeVerificationInfo(address user)
        external
        view
        returns (
            IncomeRange incomeRange,
    ...
    function getIncomeRangeStats() external view returns (uint256[5] memory rangeCounts, uint256 totalVerified) {
    ...
    function _verifyOracleSignature(address user, bool success, bytes memory signature) private pure returns (bool) {
