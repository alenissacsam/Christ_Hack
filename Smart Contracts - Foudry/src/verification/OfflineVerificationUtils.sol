...
library OfflineVerificationUtils {
    ...
    function validateCredential(
        ...
        VerificationContext storage context
        ...
        bool signatureValid = verifyCredentialSignature(
            credential,
    ...
    function calculateConfidence(
        ...
        if (credential.expiresAt > 0) {
            uint256 timeToExpiry = credential.expiresAt > block.timestamp
                ...
                : 0;
            uint256 totalValidPeriod = credential.expiresAt -
                credential.issuedAt;
    ...
    function batchValidateCredentials(
        OfflineCredential[] memory credentials,
        VerificationContext storage context
    ...
    function getCredentialSummary(
        OfflineCredential memory credential
    ) internal view returns (string memory summary) {
        return
            string(
                abi.encodePacked(
                    "Type: ",
                    ...
                    credential.expiresAt > 0
                        ? uint256ToString(credential.expiresAt)
                        : "Never",
                    ", Issuer: ",
